use ::{
    anchor_lang::{
        prelude::*,
        solana_program::{ instruction::Instruction, secp256k1_program::ID as SECP256K1_ID },
    },
    std::convert::TryInto,
};

use crate::DcarbonError;
...
pub fn verify_secp256k1_instruction(
    ...
    if
        ix.program_id != SECP256K1_ID || // The program id we expect
        ix.accounts.len() != 0 || // With no context accounts
        ix.data.len() != 12 + 20 + 64 + 1 + msg.len() // And data of this size
    {
        return Err(DcarbonError::SigInstructionFailed.into()); // Otherwise, we can already throw err
    }
...
fn check_secp256k1_data(
    ...
    {
        return Err(DcarbonError::SigHeaderFailed.into());
    }

    // verify signature
    if
        data_eth_address != eth_address ||
        data_sig != sig ||
        data_recovery_id != &[recovery_id] ||
        data_msg != msg
    {
        return Err(DcarbonError::SigVerificationFailed.into());
    }
